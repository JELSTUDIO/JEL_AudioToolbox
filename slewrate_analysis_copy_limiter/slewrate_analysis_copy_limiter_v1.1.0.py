...
class SlewEngine:
    ...
    def measure_slew_rate(self, file_path):
        with sf.SoundFile(file_path) as f:
            ...
            avg_energy = np.zeros(channels)  # NEW: Accumulate for RMS calculation
            total_frames = 0
            ...
            for block in f.blocks(blocksize=block_size, dtype='float64'):
                ...
                # NEW: Accumulate squared energy for average (RMS) level
                avg_energy += np.sum(block**2, axis=0)

                prev_sample = block[-1]  # Save last sample for next iteration

            if total_frames == 0 or np.all(peak_amps == 0):
                return None, None, None

            # NEW: Compute RMS average level per channel
            avg_amps = np.sqrt(np.divide(avg_energy, total_frames))

            # Keep backward compat: store ratio relative to peak
            ...
            metadata = {
                ...
                "avg_amps": avg_amps.tolist(),  # NEW: Stored in preset
    ...
    def apply_slew_limit(self, target_path, slew_ratio_arr, slew_abs_arr, mode, apply_mode, oversample_factor, output_path, guard_band, ram_limit_gb, progress_callback=None, global_sample_adder=None, knee_fraction=0.35, stopband_db=96.0):
        ...
        with sf.SoundFile(target_path) as infile:
            ...
            # ─── DYNAMIC REFERENCE AMPLITUDE SCANNING ────────────────────────
            if mode == "peak":
                report("upsampling", 0, "Scanning target peak amplitude...")
                ref_amps = np.zeros(num_channels)
                for block in infile.blocks(blocksize=fs*2, dtype='float64'):
                    if block.ndim == 1: block = block[:, np.newaxis]
                    ref_amps = np.maximum(ref_amps, np.max(np.abs(block), axis=0))
                report("upsampling", 100, "Peak scan complete.")
            elif mode == "avg":
                report("upsampling", 0, "Scanning target average amplitude...")
                avg_energy = np.zeros(num_channels)
                scan_frames = 0
                for block in infile.blocks(blocksize=fs*2, dtype='float64'):
                    if block.ndim == 1: block = block[:, np.newaxis]
                    n_block = block.shape[0]
                    scan_frames += n_block
                    avg_energy += np.sum(block**2, axis=0)
                ref_amps = np.sqrt(np.divide(avg_energy, max(scan_frames, 1)))
                report("upsampling", 100, "Average scan complete.")
            else:  # mode == "fs" (0 dBFS)
                ref_amps = np.ones(num_channels)

            target_slew_per_ch = s_abs * ref_amps
            # ────────────────────────────────────────────────────────────────
...
class SlewGUI:
    ...
    def setup_ui(self):
        ...
        # 👇 NEW RADIO BUTTON ADDED HERE
        ttk.Radiobutton(mode_frame2, text="Relative to Average Level (Adaptive to loudness)", variable=self.slew_mode, value="avg").pack(anchor=tk.W)
